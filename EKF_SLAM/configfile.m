...
GATE_AUGMENT= 16.0; % minimum distance for creation of new feature
% For 2-D observation:
...
% waypoint proximity
AT_WAYPOINT= 0.2; % metres, distance from current waypoint at which to switch to next waypoint
NUMBER_LOOPS= 1; % number of loops through the waypoint list

%Bumpsensor
BUMPSENSOR_RANGE=0.5;

%Timestpes to plot
timestep_Plot=10; %seconds

%intervall in observations to plot the situation (Corr, Cov....)
plot_situation_intervall=20;
...
SWITCH_WALLS=1; %if 0: no walls

SWITCH_LOG_SITUATION=1; %if 1 log and plot the situation (Correlation, Variance....)
SWITCH_PLOT_SITUATION=1;

SWITCH_EMOTION_ENGINE=1;
