...
%
function PlotSlamMap( movieName )
%PLOTSLAMMAP 
...
h.pathMark=plot(path(1,pathMark),path(2,pathMark),'r.','markersize',6);
h.path=plot(path(1,1:pathMark(end)),path(2,1:pathMark(end)),'k.','markersize',2); % vehicle path estimate



h.l=line([0 0], [0 0], 'Linewidth', 2, 'Color','k');
h.pm=line([0 0], [0 0], 'Linewidth', 2, 'Color','r');
...
h.leg=legend([h.lm h.lm_cov h.l h.pm], 'Landmarken', 'Fehlerellipsen', 'Weg des Vehikels', 'Aktuelle Situation');
...
print(h.fig1,'-dpdf','-r1200',['C:\Users\no98tey\Documents\MATLAB\Diplomarbeit\Vortrag\' movieName '\SlamMap' name '.pdf']); 
